<template>
    <div>
        <table border="1">
            ...
            <tr v-if="!codeDetails || (Array.isArray(codeDetails) && codeDetails.lengh === 0)">
                <td colspan="5">
                    List is empty.
                </td>
            </tr>
            ...
            <tr v-else v-for="codeDetail in codeDetails" :key="`${cpdeDetail.groupCode}-${codeDetail.codeValue}`">
                ...
                <td align="left">
                    <router-link :to="{ name: 'CodeDetailReadPage', params: { groupCode: codeDetail.groupCode, codeValue: codeDetail.codeValue }}">
                        {{ codeDetail.codeName }}
                    </router-link>
                </td>
